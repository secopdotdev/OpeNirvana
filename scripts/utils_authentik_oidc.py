...
Tandoor, Jellyfin, Immich, Vikunja, Komodo and writes the generated
...
from utils import EnvFile, AuthentikClient, red, green, yellow, step, container_state, resolve_admin_token
...
def ensure_email_verified_claim(ak: AuthentikClient, email_scope_pk: str) -> None:
    ...
    Some OIDC clients reject the entire OAuth response when the email_verified
    claim is false. Authentik's default email scope asserts True;
...
def write_output(path: Path, nc: dict, td: dict, jf: dict, im: dict, vk: dict, km: dict) -> None:
    text = f"""\
OIDC Setup Output
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
KOMODO  (no manual steps — restart-and-done)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Komodo reads OIDC config from environment variables at startup.
Credentials were written to .env and Komodo was restarted
automatically. No further steps are needed.

If a manual restart is required:
  docker compose up -d --no-deps --force-recreate komodo-core

Credentials:
  Client ID:     {km["client_id"]}
  Client Secret: {km["client_secret"]}
  Issuer URL:    {km["issuer"]}
  Redirect URI:  {km["redirect_uri"]}
"""
    try:
        path.write_text(text)
        step(f"Output file written to {path}")
    except OSError as exc:
        # The summary is a non-critical convenience artifact and OIDC provisioning
        # has already succeeded by this point. A non-writable path (e.g. a stale
        # root-owned file from a prior root-context run) must not fail the step.
        yellow(f"  could not write OIDC summary to {path}: {exc} "
               "(provisioning already completed successfully)")
...
def run(args, env, sync_only: bool = False) -> int:
    ...
    km_sub       = env.get("KOMODO_SUBDOMAIN")     or "komodo"
    ...
    try:
        ...
        if "401" in str(exc) or "403" in str(exc):
            ...
            # Persist the recovered token as AUTHENTIK_API_TOKEN (the re-mintable
            # scoped service token — ADR-0018) so future runs pick it up via
            # resolve_admin_token() without needing the container fallback again.
            # NEVER write to AUTHENTIK_BOOTSTRAP_TOKEN: that key is immutable and
            # revocable (ADR-0017); writing here could resurrect a deliberately
            # revoked secret.
            env.force_set("AUTHENTIK_API_TOKEN", token)
        else:
    ...
    _restart_cred_vars = {
        ...
        "komodo-core": "KOMODO_OIDC_CLIENT_ID",
    }
    ...
    # ── Komodo ────────────────────────────────────────────────────────────────
    step("Provisioning Komodo")
    # Komodo reads OIDC config from environment variables at startup.
    # Redirect URI is fixed at /auth/oidc/callback (Komodo source: auth/oidc.go).
    km_redirect = f"https://{km_sub}.{public_fqdn}/auth/oidc/callback"
    km_issuer   = f"https://{auth_sub}.{public_fqdn}/application/o/komodo/"
    km_id, km_secret, km_app_pk = provision_service(
        ak, env,
        slug="komodo", name="Komodo",
        redirect_uris=[{"matching_mode": "strict", "url": km_redirect}],
        auth_flow_pk=auth_flow, invalidation_flow_pk=invalidation_flow,
        signing_key_pk=signing_key, property_mappings=oidc_scope_mappings,
        id_var="KOMODO_OIDC_CLIENT_ID", secret_var="KOMODO_OIDC_CLIENT_SECRET",
    )
    env.set_if_blank("KOMODO_OIDC_ISSUER", km_issuer)
    km_info = {
        "client_id":     km_id,
        "client_secret": km_secret,
        "issuer":        env.get("KOMODO_OIDC_ISSUER") or km_issuer,
        "redirect_uri":  km_redirect,
    }
    ...
    #
    # Covers all apps (including the forward-auth outpost and non-OIDC apps)
    ...
    for svc in ("nextcloud", "tandoor", "vikunja", "komodo-core"):
    ...
    write_output(output_path, nc_info, td_info, jf_info, im_info, vk_info, km_info)
