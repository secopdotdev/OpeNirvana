...
| URL | Auth |
|-----|------|
| `https://health.{PUBLIC_FQDN}` | Authentik forward-auth (session required). |
| `http://health.{TAILNET_FQDN}` | Authentik forward-auth (session required). |

## Environment variables

| Variable | Description |
|----------|-------------|
| `KAFKA_SUBDOMAIN` | External subdomain slug (default: `health`) |
